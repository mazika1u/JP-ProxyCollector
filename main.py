...
DEFAULT_USER_AGENT = "JP-ProxyCollector/1.0 (+https://example.local/)"
...
def main():
    p = argparse.ArgumentParser(description="JP Proxy Collector")
    p.add_argument("--sources", required=True, help="ソースURLリストファイル or URL")
    p.add_argument("--out", default="proxies_all.txt", help="抽出した全プロキシ出力")
    p.add_argument("--out-jp", default="proxies_jp.txt", help="JP判定されたプロキシ出力")
    p.add_argument("--concurrency", type=int, default=50, help="同時検証数")
    p.add_argument("--timeout", type=int, default=8, help="検証タイムアウト秒")
    p.add_argument("--user-agent", default=DEFAULT_USER_AGENT, help="User-Agent")
    p.add_argument("--min-delay", type=float, default=1.0, help="各ソース間の待機秒")
    args = p.parse_args()
    ...
    for url in urls:
        ...
        if not allowed_by_robots(url, args.user_agent):
            print(f"[WARN] robots.txt で制限されている可能性: {url}")
            continue
